      ...
      try {
        if (user_id) {
          // const response = await fetch(
            ...
            const response = await fetch(`https://usermgtapi3.onrender.com/api/get_user/${user_id}`);
              ...
              <div className="mx-auto grid max-w-4xl grid-cols-1 gap-4 sm:md:grid-cols-2">
                {userData.profile_links.map((link, index) => {
                  try {
                    ...
                    return (
                      <SocialLink key={index} href={parsedLink.url} label={parsedLink.type} icon={iconMap[parsedLink.type]} />
                    );
                  } catch (error) {
